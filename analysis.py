...
def plot_file(file, current_problems: (str, str)):
    old_problem, new_problem = current_problems
    ...
    with open(f"./data/{new_problem}.csv") as f2:
        ...
        for row2 in reader2:
            ...
            new_problem_y_err.append(stdev)
            new_problem_y.append(statistics.mean(float_row2))
    ...
    plt.bar(X_axis, y, 0.4, label=old_problem)
    plt.errorbar(X_axis, y, y_err, fmt=".", color="Black", elinewidth=2, capthick=10, errorevery=1, alpha=0.5, ms=4,
                 capsize=2)

    plt.bar(X_axis - 0.4, new_problem_y, 0.4, label=new_problem)
    plt.errorbar(X_axis - 0.4, new_problem_y, new_problem_y_err, fmt=".", color="Black", elinewidth=2, capthick=10, errorevery=1,
                 alpha=0.5, ms=4,
                 capsize=2)

    name = old_problem.replace("_", " ")
    if old_problem == CurrentProblem.TEST_AND_SET_LOCK.value:
        name = old_problem.replace("_", " ") + " & test and test and and set"
    ...
    plt.savefig(f'./data/figure_{old_problem}.png', dpi=400, transparent=False)
    # plt.show()
